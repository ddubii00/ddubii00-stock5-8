...
export default function App() {
  ...
  return <div className="app"><header className="app-header"><strong className="app-title">stock5-8</strong><div className="market-summary" aria-label="시장 실시간 시세">{MARKET_TILES.map(tile => <MarketTicker key={tile.key} label={tile.label} quote={marketSummary[tile.key]} />)}</div><div className="header-actions"><button className={state.mode === 'KRX' ? 'active' : ''} onClick={() => save({ ...state, mode: 'KRX' })}>KRX</button><button className={state.mode === 'KRX2' ? 'active' : ''} onClick={() => save({ ...state, mode: 'KRX2' })}>KRX 장후</button><button onClick={() => setModal(true)}>종목</button></div></header>{ordered.length ? <div className="dashboard-grid watch-dashboard">{ordered.map(item => <div className="watch-chart" key={item.id}><div className="watch-category">{item.group}</div><ChartColumn id={`watch-${item.id}`} defaultSymbol={item.symbol} defaultName={item.name} marketMode={state.mode} memo={item.memo} memoPosition={item.memoPosition} memoSize={item.memoSize} onMemoChange={(memo, memoPosition, memoSize) => save({ ...state, items: state.items.map(row => row.id === item.id ? { ...row, memo, memoPosition, memoSize } : row) })} /></div>)}</div> : <main className="empty-state"><h2>관심 종목을 추가하세요</h2><p>상단의 ‘종목’ 버튼에서 카테고리를 고르고 종목·지수를 검색할 수 있습니다.</p><button onClick={() => setModal(true)}>종목 추가</button></main>}{modal && <WatchlistModal state={state} onChange={save} onClose={() => setModal(false)} />}</div>;
}
